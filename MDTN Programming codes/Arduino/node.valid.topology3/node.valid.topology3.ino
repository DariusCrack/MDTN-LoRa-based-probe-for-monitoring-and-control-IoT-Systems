...
// 3) Callback con firma osjobcb_t
void onSendJob(osjob_t* j) {
  // aquí llamas a tu función con los datos “guardados”
  ...
  while (Serial1.available()) {
    ...
    switch (state) {
      ...
      case READ_CHK:
        ...
        state = WAIT_SOF;
        // … tras validar checksum y extraer payloadBuf/payloadLen …
        memcpy(lastPayload, payloadBuf, payloadLen);
        lastPayloadLen = payloadLen;

        // programa envío dentro de TX_INTERVAL segundos
...
void loop2( void * parameter )
{
  ...
  Serial.println("[UART Task] UART2 initialized on GPIO45 RX / GPIO46 TX");
  Serial2.println("Hi?");
  for(;;){
...
void setup () {
  ...
  Serial.println(F("UART1 configured on IO42 (RX) / IO41 (TX)"));
  ...
  Serial.println("Serial1 listo en IO42 (RX) / IO41 (TX)");
...
float readInternalTemperature() {
  ...
  // The ESP32 internal temperature sensor gives an approximate range of 0-100°C
    ...
    tempe= (0.4386 * tempe)-(27.88*offsetval)-20.52;
    return tempe;  
}
